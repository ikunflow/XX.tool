...
const Utils = (function () {
     ...
     */
    function compressEngine(canvas, quality, maxBytes, callback) {
        canvas.toBlob(function (blob) {
            if (blob.size <= maxBytes || quality <= 0.1) {
                if (blob.size > maxBytes) {
                    ...
                    compressEngine(shrink, 0.9, maxBytes, callback);
                } else {
                    callback(blob);
                }
            } else {
                compressEngine(canvas, quality - 0.05, maxBytes, callback);
            }
        }, 'image/jpeg', quality);
    }
